...
+(UIImage * _Nonnull)watermarkImage:(NSString *_Nonnull)watermarkStr image:(NSString*_Nonnull)image{
    ...
    UIImage *oldImag = [UIImage imageNamed:image];
    //开启上下文
    //size新图片的大小
    //opaque YES不透明 NO透明
    UIGraphicsBeginImageContextWithOptions(oldImag.size, NO, 0.0);
    [oldImag drawAtPoint:CGPointZero];
    NSString *str = watermarkStr;
    NSDictionary *dict = @{
                           NSFontAttributeName : [UIFont boldSystemFontOfSize:14],
                           NSForegroundColorAttributeName : [UIColor whiteColor]
                           };
    if (str.length>=2) {
        [str drawAtPoint:CGPointMake(oldImag.size.width/4-str.length, oldImag.size.height/4-3) withAttributes:dict];
    }else{
        [str drawAtPoint:CGPointMake(oldImag.size.width/4+2, oldImag.size.height/4-3) withAttributes:dict];
    }
    //获取新的图片
    UIImage *newImage = UIGraphicsGetImageFromCurrentImageContext();
    //关闭上下文
