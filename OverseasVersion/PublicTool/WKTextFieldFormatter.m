...
- (void)textChanged:(UITextField *)textField {
    ...
    switch (_formatterType) {
        ...
        {
            regexString = [NSString stringWithFormat:@"^(\\d+)\\.?(\\d{0,%lu})$", (unsigned long)_decimalPlace];
            break;
        }
        ...
        {
            regexString = [NSString stringWithFormat:@"^[%@]{0,%lu}$", _characterSet, (unsigned long)_limitedLength];
            break;
        }
        default:
            break;
    ...
    !_backStr ? : _backStr (textField.text);
}
