...
-(JWScrollviewCell *)userNameCell{
    ...
    if (!_userNameCell) {
        _userNameCell = [[JWScrollviewCell alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight)];
        ...
        JWLabel * numLab = [[JWLabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(imageView.frame), imageView.y, 45, 25)];
...
//    [CommonService getVerifyCodeWithMobile:((JWTextField *)self.userNameCell.getElementByTag(2000)).text accountToken:self.defaultSetting.access_token success:^(AFHTTPRequestOperation *operation, id responseObject) {
//        HXStrong_self
//        if ([[responseObject objectForKey:@"errorCode"] integerValue] == 0) {
////            [self showTip:[TIPTEXT sendVerificationCode]];
//            [self pushVerificationCode];
...
-(void)getForgetPasswordVerificationCode{
    ...
    [CommonService getForgetPasswordVerifyCodeWithMobile:((JWTextField *)self.userNameCell.getElementByTag(2000)).text
                                            accountToken:self.defaultSetting.access_token
                                               area_code:((JWLabel *)self.userNameCell.getElementByTag(1998)).text
                                                 success:^(AFHTTPRequestOperation *operation, id responseObject) {
         HXStrong_self
...
//    
//    [CommonService getForgetPasswordVerifyCodeWithMobile:((JWTextField *)self.userNameCell.getElementByTag(2000)).text accountToken:self.defaultSetting.access_token success:^(AFHTTPRequestOperation *operation, id responseObject) {
//        HXStrong_self
//        if ([[responseObject objectForKey:@"errorCode"] integerValue] == 0) {
////            [self showTip:[TIPTEXT sendVerificationCode]];
//            [self pushVerificationCode];
