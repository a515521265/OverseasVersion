...
#define CELL_MARGIN_TB  10
#define CELL_MARING_LR  10
...
+ (instancetype) messageWihtContent:(NSString *)content isRight:(BOOL)isRight {
    ...
    if (msg.isRight) {
        ...
        msg.headerFrame = CGRectMake(SCREENWIDTH - adaptY(30)-10, frameOfPop.size.height - 25, adaptY(30), adaptY(30));
    } else {
        ...
        bounds.size.height = frameOfPop.size.height +2 * CELL_MARGIN_TB;
        msg.bounds = bounds;
        msg.headerFrame = CGRectMake(10, frameOfPop.size.height - 25, adaptY(30), adaptY(30));
    }
    return msg;
}
...
//构造消息对象 -- 消息图片
+ (instancetype) messageWihtImageStr:(NSString *)imageStr isRight:(BOOL)isRight{
    
    Message *msg = [Message new];
    msg.isRight = isRight;
    msg.imageStr = imageStr;
    CGRect rect = CGRectMake(0, 0, 120, 120);//[msg.content boundingRectWithSize:CGSizeMake(200, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14]} context:nil];
    CGRect frameOfLabel = CGRectZero;
