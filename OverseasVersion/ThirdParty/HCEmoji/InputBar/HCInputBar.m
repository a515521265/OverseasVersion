...
- (instancetype)initWithStyle:(InputBarStyle)style {
    ...
    self = [super initWithFrame:CGRectMake(0, ScreenHeight-KNavHeight-height, ScreenWidth, height)];
...
- (void)drawRect:(CGRect)rect {
    ...
    }else if (_style == CustomInputBarStyle){
...
-(UIButton *)sendBtn{
    ...
    if (!_sendBtn) {
        _sendBtn = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.inputView.frame) + kKeyboardX, kKeyboardY, kKeyboardWidth, kKeyboardWidth)];
        _sendBtn.tag = 19;
        [_sendBtn setImage:[UIImage imageNamed:@"send-1"] forState:UIControlStateNormal];
        [_sendBtn addTarget:self action:@selector(ClickedKeyboard:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _sendBtn;
...
- (UITextView *)inputView {
    if (!_inputView) {
        if (_style == ExpandingInputBarStyle) {
            ...
            _inputView = [[UITextView alloc]initWithFrame:CGRectMake(kInputViewOtherX, kInputViewY, (ScreenWidth-kKeyboardWidth*1-4*kKeyboardX), kInputViewHeight)];
        }else{
...
//        _inputView.tintColor = [UIColor clearColor];
        [self setBorderinView:_inputView CornerRadius:kInputViewHeight/2 Color:[UIColor borderColor]];
    }
    return _inputView;
}
...
- (void)keyboardWillShow:(NSNotification *)notification
{
    ...
    [UIView animateWithDuration:[info[UIKeyboardAnimationDurationUserInfoKey] doubleValue]
                     ...
                     animations:^{
                         ...
                         newInputViewFrame.origin.y = [UIScreen mainScreen].bounds.size.height-CGRectGetHeight(self.frame)-kbSize.height - KNavHeight;
...
- (void)keyboardWillHidden:(NSNotification *)notification {
    ...
    [UIView animateWithDuration:[info[UIKeyboardAnimationDurationUserInfoKey] doubleValue]
                        ...
                        options:([info[UIKeyboardAnimationCurveUserInfoKey] integerValue]<<16)
                     animations:^{
                         self.center = CGPointMake(self.bounds.size.width/2.0f, height-CGRectGetHeight(self.frame)/2.0 - KNavHeight);
                     }
                     completion:nil];
}
